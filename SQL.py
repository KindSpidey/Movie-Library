...
class WorkingBD():
    ...
    def add_screenwriter(name, phone, email):
        ...
        conn.commit()
        conn.close()
    def add_film(title, box_office, rating, year, director_name, screenwriter_name, composer_name, *actors_names):
        conn = sqlite3.connect('Movies.db')
        cursor = conn.cursor()
        query = '''
                    INSERT INTO film(title, box_office,rating, year, director_name, screenwriter_name,composer_name)
                                VALUES (?,?,?,?,?,?,?)
                '''
        cursor.execute(query, (title, box_office, rating, year, director_name, screenwriter_name,composer_name))
        conn.commit()
    ...
    def get_film_by_id(ids):
        ...
        cursor = conn.cursor()
        query = f'''
                    SELECT title, box_office, rating, year, director_name, screenwriter_name, composer_name
                    FROM film
                    WHERE id = {ids!r}
    ...
    def remove_actor_by_name(name1):
        ...
        cursor = conn.cursor()
        query = f'''
                    DELETE FROM actor WHERE name = "{name1}"
                '''
        cursor.execute(query)
        conn.commit()
        conn.close()
    ...
    def remove_composer_by_name(name1):
        ...
        cursor = conn.cursor()
        query = f'''
                    DELETE FROM composer WHERE title = "{name1}"
                '''
        cursor.execute(query)
        conn.commit()
        conn.close()
...
WorkingBD.add_film('Spider-Man',821708551,90,2002,'Sam Raimi','David Koepp','Danny Elfman','Tobey Maguire', 'Kirsten Dunst', 'Willem Dafoe','James Franco', 'J.K. Simmons', 'Joe Manganiello')
